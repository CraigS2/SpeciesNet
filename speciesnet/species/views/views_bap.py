...
def bapSubmission(request, pk):
    ...
    logger.info(
        "User %s viewed bapSubmission: %s (%s)", request.user.username, bap_submission.name, str(bap_submission.id)
    )
...
def createBapSubmission(request, pk):
    ...
    logger.info(
        "request.session['species_instance_id'] retrieved for bapSubmission: %s",
    ...
    try:
        ...
        logger.error(
            "User %s creating bapSubmission for club %s:  multiple BapSpecies entries found",
    ...
    if bap_points == 0:
        ...
        if species_name and " " in species_name:
            ...
            try:
                ...
                logger.warning(
                    "User %s creating bapSubmission for club %s: No BapGenus entry found:  %s.  Club default points used.",
                    request.user.username,
                    club.name,
                    genus_name,
                )
                ...
                messages.error(request, error_msg)
                logger.error(
                    "User %s creating bapSubmission for club %s:  Multiple BapGenus entries found:  %s.",
                    request.user.username,
                    club.name,
                    genus_name,
                )
            ...
            messages.error(request, error_msg)
            logger.error(
                "User %s creating bapSubmission for club %s: species %s failed to resolve genus name.",
                request.user.username,
                club.name,
                species_name,
            )
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if not bapGenusFound and genus_name is not None:
                    ...
                    bap_submission.admin_comments = (
                        "Genus points not configured. Default club points applied.  Please review."
                    )
                ...
                logger.info(
                    "User %s created bapSubmission for club:  %s (%s)", request.user.username, club.name, str(club.id)
                )
...
def editBapSubmission(request, pk):
    ...
    if not (userIsBapAdmin or request.user.is_staff) and not bap_submission.aquarist == request.user:
        raise PermissionDenied
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            logger.info(
                "User %s edited bapSubmission:  %s (%s)",
                request.user.username,
                bap_submission.name,
                str(bap_submission.id),
            )
            return HttpResponseRedirect(reverse("bapSubmission", args=[bap_submission.id]))
...
def deleteBapSubmission(request, pk):
    ...
    if request.method == "POST":
        logger.info(
            "User %s deleted bapSubmission: %s (%s)", request.user.username, bap_submission.name, str(bap_submission.id)
        )
...
class BapSubmissionsView(LoginRequiredMixin, ListView):
    ...
    def get_bap_club(self):
        ...
        return AquaristClub.objects.get(id=bap_club_id)
    ...
    def get_context_data(self, **kwargs):
        ...
        context["userCanEdit"] = (
            user_can_edit_club(self.request.user, self.get_bap_club()) or self.request.user.is_staff
        )
...
class BapLeaderboardView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        bap_submissions = BapSubmission.objects.filter(
            club=bap_club, year=year, status=BapSubmission.BapSubmissionStatus.APPROVED
        )
        ...
        for aquarist_id in aquarist_ids:
            ...
            for bap_submission in cur_aquarist_submissions:
                ...
                print(
                    "  BAP Leaderboard Entry updated: "
                    + bap_leaderboard_entry.name
                    + " ("
                    + str(bap_leaderboard_entry.points)
                    + ")"
                )

            bap_leaderboard_entry.save()
            print(
                "BAP Leaderboard Entry finalized: "
                + bap_leaderboard_entry.name
                + " ("
                + str(bap_leaderboard_entry.points)
                + ")"
        ...
        return BapLeaderboard.objects.filter(club=bap_club, year=year).order_by("-points")
    ...
    def get_context_data(self, **kwargs):
        ...
        context["bap_club"] = self.get_bap_club()
        record_page_view(
            PageViewCount.PageType.BAP_LEADERBOARD, self.get_bap_club().id, self.request.user.is_authenticated
        )
        return context
...
class BapGenusView(LoginRequiredMixin, ListView):
    ...
    def post(self, request, pk):
        ...
        for species in species_set:
            ...
            if genus_name not in current_genus_names and genus_name not in new_genus_names:
        ...
        if len(new_genus_names) > 0:
            ...
            logger.info(
                "Update of bapGenus list complete for %s:  New genus count: %s", club.name, len(new_genus_names)
            )
            ...
            print("BapGenus set update - no new genus entries found")
        ...
        for species in species_set:
            ...
            if species_name and " " in species_name:
                ...
                if genus_name not in genus_names:
                    ...
                    bapGP = BapGenus(
                        name=genus_name, club=club, example_species=species, points=club.bap_default_points
                    )
                    bapGP.save()
...
class BapGenusSpeciesView(LoginRequiredMixin, ListView):
    ...
    def get_species_without_bsp_overrides(self):
        ...
        logger.info(
            "request.session['bap_genus_id'] set for create/edit/delete BapSpecies by BapGenusSpeciesView:  %s",
            str(bgp.id),
        )
        ...
        try:
            BapGenus.objects.get(name=genus_name, club=club)
        except ObjectDoesNotExist:
...
def createBapSpecies(request, pk):
    ...
    logger.info(
        "request.session['bap_genus_id'] retrieved for createBapSpecies: %s", str(request.session["bap_genus_id"])
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            bapSP.save()
            logger.info(
                "User %s created bapSpecies for species: %s (%s)", request.user.username, species.name, str(species.id)
            )
...
def deleteBapSpecies(request, pk):
    ...
    bgp_id = request.session["bap_genus_id"]
    logger.info(
        "request.session['bap_genus_id'] retrieved by deleteBapSpecies:  %s", str(request.session["bap_genus_id"])
    )

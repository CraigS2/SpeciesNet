    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                messages.success(
                    request, "Thank you! Your feedback has been submitted and will be reviewed by our staff."
                )
...
def applySpeciesFeedbackPhoto(request, pk):
    ...
    if request.method == "POST":
        if feedback.species_image:
            ...
            logger.info(
                "Staff user %s applied photo from feedback %s to species: %s",
                request.user.username,
                pk,
                feedback.species.name,
            )
...
def archiveSpeciesFeedback(request, pk):
    ...
    if request.method == "POST":
        ...
        feedback.save()
        logger.info(
            "Staff user %s archived feedback %s for species: %s", request.user.username, pk, feedback.species.name
        )
...
def deleteSpeciesFeedback(request, pk):
    ...
    if request.method == "POST":
        ...
        feedback.delete()
        logger.info(
            'Staff user %s deleted feedback "%s" for species: %s', request.user.username, feedback_name, species_name
        )

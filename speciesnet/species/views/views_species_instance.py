...
def speciesInstance(request, pk):
    ...
    if isBapParticipant:
        ...
        if bapClubMemberships.count() > 0:
            ...
            logger.info(
                "request.session['species_instance_id'] set for bapSubmission by speciesInstance:   %s",
                str(speciesInstance.id),
            )
            for membership in bapClubMemberships:
                try:
                    bapSubmission = BapSubmission.objects.get(
                        club=membership.club, aquarist=speciesInstance.user, speciesInstance=speciesInstance
                    )
        ...
        try:
            ...
            caresRegistration = (
                CaresRegistration.objects.filter(
                    species=speciesInstance.species, aquarist_email=speciesInstance.user.email
                )
                .exclude(status=CaresRegistration.CaresRegistrationStatus.CLOSED)
    ...
    if request.user.is_authenticated:
        logger.info(
            "User %s visited aquarist species page:  %s (%s).",
            request.user.username,
            speciesInstance.name,
            speciesInstance.user.username,
        )
    else:
        logger.info(
            "Anonymous user visited aquarist species page:  %s (%s).",
            speciesInstance.name,
            speciesInstance.user.username,
        )
        ...
        if form.is_valid():
            try:
                ...
                if speciesInstance.aquarist_species_video_url:
                    speciesInstance.aquarist_species_video_url = processVideoURL(
                        speciesInstance.aquarist_species_video_url
                    )
                    speciesInstance.save()
                logger.info(
                    "User %s added speciesInstance:  %s (%s)",
                    request.user.username,
                    speciesInstance.name,
                    str(speciesInstance.id),
                )
        ...
        if form.is_valid():
            try:
                ...
                if speciesInstance.aquarist_species_video_url:
                    ...
                    speciesInstance.save()
                logger.info(
                    "User %s edited speciesInstance: %s (%s)",
                    request.user.username,
                    speciesInstance.name,
                    str(speciesInstance.id),
                )
...
def deleteSpeciesInstance(request, pk):
    ...
    if request.method == "POST":
        ...
        logger.info(
            "User %s deleted speciesInstance: %s (%s)",
            request.user.username,
            speciesInstance.name,
            str(speciesInstance.id),
        )
...
def reassignSpeciesInstance(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            speciesInstance.save()
            logger.info(
                "User %s reassigned species (%s) to (%s) for speciesInstance: %s (%s)",
                request.user.username,
                str(cur_species.id),
                str(new_species.id),
                speciesInstance.name,
            ...
            messages.success(
                request,
                f'Aquarist Species "{speciesInstance.name}" updated to use new species "{new_species.name}" successfully!',
            )
            return HttpResponseRedirect(reverse("speciesInstance", args=[speciesInstance.id]))
        ...
        if form.is_valid():
            try:
                ...
                if species:
                    ...
                    messages.success(
                        request, f'Successfully created species "{species.name}" and your Aquarist Species!'
                    )
                    logger.info(
                        "User %s added species: %s (%s) and speciesInstance: %s (%s)",
                        request.user.username,
                        species.name,
                        str(species.id),
                        speciesInstance.name,
...
def speciesInstanceLog(request, pk):
    ...
    if request.user.is_authenticated:
        logger.info(
            "User %s visited aquarist species log:   %s (%s).",
            request.user.username,
            speciesInstance.name,
            speciesInstance.user.username,
        )
        ...
        logger.info(
            "Anonymous user visited aquarist species log: %s (%s).", speciesInstance.name, speciesInstance.user.username
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if speciesInstanceLogEntry.log_entry_video_url:
                speciesInstanceLogEntry.log_entry_video_url = processVideoURL(
                    speciesInstanceLogEntry.log_entry_video_url
                )
            speciesInstanceLogEntry.save()
            speciesInstanceLogEntry.speciesInstance.save()  # Update timestamp
            logger.info(
                "User %s created new speciesInstanceLogEntry for %s (%s)",
                request.user.username,
                speciesInstance.name,
                str(speciesInstance.id),
            )
...
def editSpeciesInstanceLogEntry(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            speciesInstanceLogEntry.speciesInstance.save()  # Update timestamp
            logger.info(
                "User %s edited speciesInstanceLog for %s (%s)",
                request.user.username,
                speciesInstance.name,
                str(speciesInstance.id),
            )
        ...
        for si in species_choices:
            ...
            if si_labels.count() > 0:
                ...
                si_label = SpeciesInstanceLabel(
                    name=name, text_line1=text_line1, text_line2=text_line2, speciesInstance=speciesInstance
                )
...
def registerCaresSpeciesInstance(request, pk):
    ...
    if cares_species.cares_classification == Species.CaresStatus.NOT_CARES_SPECIES:
        messages.error(
            request, f'"{cares_species.name}" is not classified as a CARES species and cannot be registered.'
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                messages.error(
                    request, "A registration conflict occurred (possibly a duplicate). Please contact support."
                )
            except Exception as e:

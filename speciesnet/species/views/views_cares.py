            ...
            try:
                ...
                if species.cares_classification != Species.CaresStatus.NOT_CARES_SPECIES:
                    ...
                    if not Species.objects.filter(name=species_name).exists():
                        ...
                        logger.info(
                            "User %s created new species: %s (%s)", request.user.username, species.name, str(species.id)
                        )
                    ...
                    logger.info(
                        "User %s attempted to create duplicate species.  Redirected to:  %s",
...
def deleteCaresSpecies(request, pk):
    ...
    if speciesInstances.count() > 0:
        ...
        logger.warning(
            "User %s attempted to delete species:  %s with speciesInstance dependencies. Deletion blocked.",
            request.user.username,
            species.name,
        )
...
def registrationLookup(request):
    ...
    if request.method == "POST":
        ...
        registrations = (
            CaresRegistration.objects.filter(aquarist_email__iexact=email)
            .select_related("species", "affiliate_club", "cares_approver")
            .order_by("-date_requested")
        )

        if not registrations.exists():
            messages.warning(request, f"No registrations found for {email}")
            return render(request, "species/cares/registrationLookup.html")

        context = {"registrations": registrations, "email": email, "registration_count": registrations.count()}
        logger.info(
            "registrationLookup inquiry for " + email + " found " + str(registrations.count()) + " registrations"
        )
...
def createCaresRegistration(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            logger.info(
                "User %s created caresRegistration: %s (%s)",
                request.user.username,
                registration.name,
                str(registration.id),
            )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                registration.save()
                logger.info(
                    "User %s edited cares registration: %s (%s)",
                    request.user.username,
                    registration.name,
                    str(registration.id),
                )
                ...
                messages.error(
                    request, "The registration data conflicts with existing records (possibly duplicate name)."
                )
            except Exception as e:
            ...
            logger.warning(
                f"Cares registration form validation failed for registration_id={pk}: {form.errors.as_text()}"
            )
            messages.error(request, "Please correct the errors highlighted below.")
    else:
...
class CaresRegistrationListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = CaresRegistration.objects.select_related("cares_approver", "species", "affiliate_club")
    ...
    def get_context_data(self, **kwargs):
        ...
        approvers = (
            CaresRegistration.objects.values_list("cares_approver__id", "cares_approver__name")
            .distinct()
            .order_by("cares_approver__name")
        )
        context["approvers"] = [(approver_id, name) for approver_id, name in approvers if approver_id is not None]

        # Get unique clubs
        clubs = (
            CaresRegistration.objects.values_list("affiliate_club__id", "affiliate_club__name")
            .distinct()
            .order_by("affiliate_club__name")
        )
...
def createCaresApprover(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            cares_approver.save()
            logger.info(
                "User %s created caresApprover: %s (%s)",
                request.user.username,
                cares_approver.name,
                str(cares_approver.id),
            )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                logger.info(
                    "User %s edited cares approver: %s (%s)",
                    request.user.username,
                    cares_approver.name,
                    str(cares_approver.id),
                )

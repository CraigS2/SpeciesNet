...
def speciesMaintenanceLog(request, pk):
    ...
    speciesMaintenanceLogEntries = SpeciesMaintenanceLogEntry.objects.filter(
    ...
    logger.info(
        "User %s visited speciesMaintenanceLog %s (%s)",
        request.user.username,
        speciesMaintenanceLog.name,
        str(speciesMaintenanceLog.id),
    )
    ...
    record_page_view(
        PageViewCount.PageType.SPECIES_MAINTENANCE_LOG, speciesMaintenanceLog.id, request.user.is_authenticated
    )
...
def createSpeciesMaintenanceLog(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            logger.info(
                "User %s created speciesMaintenanceLog %s (%s)",
                request.user.username,
                speciesMaintenanceLog.name,
                str(speciesMaintenanceLog.id),
            )
...
def editSpeciesMaintenanceLog(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            speciesMaintenanceLog = form.save()
            logger.info(
                "User %s edited speciesMaintenanceLog %s (%s)",
                request.user.username,
                speciesMaintenanceLog.name,
                str(speciesMaintenanceLog.id),
            )
...
def deleteSpeciesMaintenanceLog(request, pk):
    ...
    if request.method == "POST":
        logger.info(
            "User %s deleted speciesMaintenanceLog %s (%s)",
            request.user.username,
            speciesMaintenanceLog.name,
            str(speciesMaintenanceLog.id),
        )
...
def addMaintenanceGroupCollaborator(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            logger.info(
                "User %s added speciesMaintenanceLog collaborator %s (%s)",
                request.user.username,
                speciesMaintenanceLog.name,
                str(speciesMaintenanceLog.id),
            )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for choice in user_choices:
                ...
                speciesMaintenanceLog.speciesInstances.add(speciesInstance)
            logger.info(
                "User %s added speciesMaintenanceLog speciesInstance %s (%s)",
                request.user.username,
                speciesMaintenanceLog.name,
                str(speciesMaintenanceLog.id),
            )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if speciesMaintenanceLogEntry.log_entry_video_url:
                speciesMaintenanceLogEntry.log_entry_video_url = processVideoURL(
                    speciesMaintenanceLogEntry.log_entry_video_url
                )
            ...
            logger.info(
                "User %s created speciesMaintenanceLog entry %s (%s)",
                request.user.username,
                speciesMaintenanceLog.name,
                str(speciesMaintenanceLog.id),
            )
...
def editSpeciesMaintenanceLogEntry(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            logger.info(
                "User %s edited speciesMaintenanceLog entry %s (%s)",
                request.user.username,
                speciesMaintenanceLog.name,
                str(speciesMaintenanceLog.id),
            )
            return HttpResponseRedirect(
                reverse("speciesMaintenanceLog", args=[speciesMaintenanceLogEntry.speciesMaintenanceLog.id])
            )
...
def deleteSpeciesMaintenanceLogEntry(request, pk):
    ...
    if request.method == "POST":
        ...
        logger.info(
            "User %s deleted speciesMaintenanceLog entry %s (%s)",
            request.user.username,

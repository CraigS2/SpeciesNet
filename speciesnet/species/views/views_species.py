...
class SpeciesListView(ListView):
    ...
    def get_queryset(self):
        queryset = Species.objects.annotate(instance_count=Count("species_instances"))
...
def createSpeciesReferenceLink(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            form.save()
            logger.info(
                "User %s created speciesReferenceLink for species:  %s (%s)",
                request.user.username,
                species.name,
                str(species.id),
            )
...
def editSpeciesReferenceLink(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            form.save()
            logger.info(
                "User %s edited speciesReferenceLink for species: %s (%s)",
                request.user.username,
                species.name,
                str(species.id),
            )
...
def deleteSpeciesReferenceLink(request, pk):
    ...
    if request.method == "POST":
        species = speciesReferenceLink.species
        logger.info(
            "User %s deleted speciesReferenceLink for species:  %s (%s)",
            request.user.username,
            species.name,
            str(species.id),
        )
...
def createSpeciesCollectionLocation(request, pk):
    ...
    if request.method == "POST":
        ...
        if name:
            duplicate = SpeciesCollectionLocation.objects.filter(species=species, name__iexact=name).exists()
            if duplicate:
                ...
                logger.info(
                    'User %s added collection location "%s" for species %s (%s)',
                    request.user.username,
                    name,
                    species.name,
                    species.id,
...
def editSpeciesCollectionLocation(request, pk):
    ...
    if request.method == "POST":
        ...
        if name:
            duplicate = (
                SpeciesCollectionLocation.objects.filter(species=species, name__iexact=name)
            ...
            if duplicate:
                ...
                location.save()
                logger.info(
                    'User %s edited collection location pk=%s to "%s" for species %s (%s)',
                    request.user.username,
                    location.pk,
                    name,
                    species.name,
...
def deleteSpeciesCollectionLocation(request, pk):
    ...
    if request.method == "POST":
        ...
        location.delete()
        logger.info(
            'User %s deleted collection location "%s" for species %s (%s)',
            request.user.username,
            name,
            species.name,
            species.id,

...
def _email_already_entered(email):
    return any(e.get("email", "").strip().lower() == email.strip().lower() for e in _read_entries())
...
def raffle_enter(request):
    ...
    if request.method == "POST":
        ...
        if errors:
            ...
            return render(
                request,
                "species/raffle/raffle_enter.html",
                {
...
def raffle_dashboard(request):
    ...
    for row in species_rows:
        ...
        pool = [e for e in entries if sp in (e.get("species_1", ""), e.get("species_2", ""), e.get("species_3", ""))]
        # Winners are entries whose winner_for contains this species name
        winners = [e for e in entries if sp in [w.strip() for w in e.get("winner_for", "").split("|") if w.strip()]]
        species_pools[sp] = {
    ...
    return render(
        request,
        "species/raffle/raffle_dashboard.html",
        {
            "entries": entries,
...
def raffle_upload_species(request):
    ...
    try:
        ...
        logger.info(
            "Staff %s uploaded species CSV: %d added, %d updated, %d skipped",
            request.user.username,
            added,
            updated,
            skipped,
        )
        messages.success(request, f"Species list updated — {added} added, {updated} updated, {skipped} skipped.")
...
def raffle_pick_winner(request, species_name):
    ...
    current_winners = [
        e for e in entries if species_name in [w.strip() for w in e.get("winner_for", "").split("|") if w.strip()]
    ]

    if len(current_winners) >= qty:
        messages.warning(request, f'All {qty} winner(s) for "{species_name}" have already been chosen.')
        return HttpResponseRedirect(reverse("raffle_dashboard"))
    ...
    try:
        ...
        messages.success(
            request, f'🏆 Winner for "{species_name}": {winner["first_name"]} {winner["last_name"]} ({winner["email"]})'
        )
    except Exception as e:
...
def raffle_mark_manual_winner(request, species_name, email):
    ...
    target = next((e for e in entries if e.get("email", "").lower() == email_lower), None)
    if not target:
    ...
    if species_name in already_won:
        messages.warning(
            request, f'{target["first_name"]} {target["last_name"]} is already a winner for "{species_name}".'
        )
        return HttpResponseRedirect(reverse("raffle_dashboard"))
    ...
    try:
        ...
        logger.info(
            'Manual winner marked for "%s": %s %s (%s)', species_name, target["first_name"], target["last_name"], email
        )
        messages.success(
            request, f'🏆 {target["first_name"]} {target["last_name"]} marked as winner for "{species_name}".'
        )
    except Exception as e:
...
def raffle_entries(request):
    ...
    return render(
        request,
        "species/raffle/raffle_entries.html",
        {
            "entries": entries,
            "entry_count": len(entries),
            "hide_google_login": True,
...
def raffle_reset(request):
    ...
    for path in (_entries_path(), _species_path()):
        if os.path.exists(path):
            ...
            try:
                ...
                logger.info("Staff %s archived raffle file: %s → %s", request.user.username, path, archive_path)
            except Exception as e:
    ...
    if errors:
        ...
        messages.success(
            request,
            f"Raffle reset. Archived: {', '.join(archived)}. Upload a new species CSV to start the next raffle.",
        )
    else:

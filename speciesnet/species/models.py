...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    instagram_url = models.URLField(
        max_length=200, blank=True, null=True, verbose_name="Instagram Profile"
    )  # help_text="Full Instagram URL (e.g., https://instagram.com/username)")
    facebook_url = models.URLField(
        max_length=200, blank=True, null=True, verbose_name="Facebook Profile"
    )  # help_text="Full Facebook URL (e.g., https://facebook.com/username)")
    youtube_url = models.URLField(
        max_length=200, blank=True, null=True, verbose_name="YouTube Channel"
    )  # help_text="Full YouTube URL (e.g., https://youtube.com/@username)")
...
class Species(models.Model):
    ...
    cares_classification = models.CharField(
        max_length=4, choices=CaresStatus.choices, default=CaresStatus.NOT_CARES_SPECIES
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, editable=False, null=True, related_name="user_created_species"
    )
    lastUpdated = models.DateTimeField(auto_now=True)  # updated every DB FSpec save
    last_edited_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, editable=False, null=True, related_name="user_last_edited_species"
    )
...
class SpeciesAdmin(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="species_admins"
    )  # deletes species instances if user deleted
    ...
    last_updated_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="species_admin_updaters"
    )
    created = models.DateTimeField(auto_now_add=True)
...
class SpeciesInstance(models.Model):
    name = models.CharField(max_length=240)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, editable=False, related_name="user_species_instances"
    )  # delestes Species instances if user deleted
    species = models.ForeignKey(
        Species, on_delete=models.PROTECT, null=False, related_name="species_instances"
    )  # allows Species deletion *only* if no SpeciesInstances exist
    ...
    collection_location = models.ForeignKey(
        "SpeciesCollectionLocation", on_delete=models.SET_NULL, null=True, blank=True, related_name="species_instances"
    )
    acquired_from = models.ForeignKey(
        "self", on_delete=models.SET_NULL, null=True, related_name="shared_species_instances"
    )  # self == SpeciesInstance
    year_acquired = models.IntegerField(
        null=True, blank=True, validators=[MinValueValidator(1900), MaxValueValidator(2100)], default=get_cur_year
    )  # no () on get_cur_year
...
class SpeciesInstanceLogEntry(models.Model):
    name = models.CharField(max_length=240)
    speciesInstance = models.ForeignKey(
        SpeciesInstance, on_delete=models.PROTECT, null=False, related_name="species_instance_log_entries"
    )
...
class SpeciesInstanceLabel(models.Model):
    name = models.CharField(max_length=200)
    speciesInstance = models.ForeignKey(
        SpeciesInstance, on_delete=models.CASCADE, null=False, related_name="species_instance_labels"
    )  # deletes ALL log entries referencing any deleted species instance
...
class SpeciesInstanceComment(models.Model):
    name = models.CharField(max_length=240)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, editable=False, related_name="user_species_instance_comments"
    )  # delestes species instances if user deleted
    speciesInstance = models.ForeignKey(
        SpeciesInstance, on_delete=models.CASCADE, null=False, related_name="species_instance_comments"
    )  # deletes ALL instances referencing any deleted species
...
class SpeciesMaintenanceLog(models.Model):
    name = models.CharField(max_length=240)
    species = models.ForeignKey(
        Species, on_delete=models.CASCADE, null=True, related_name="species_maintenance_logs"
    )  # deletes ALL instances referencing any deleted species
...
class SpeciesMaintenanceLogEntry(models.Model):
    name = models.CharField(max_length=240)
    speciesMaintenanceLog = models.ForeignKey(
        SpeciesMaintenanceLog, on_delete=models.CASCADE, null=False, related_name="species_maintenance_log_entries"
    )
    log_entry_image = models.ImageField(upload_to="images/%Y/%m/%d", null=True, blank=True)
    log_entry_video_url = models.URLField(max_length=500, blank=True)  # help_text="YouTube video link"
...
class AquaristClubMember(models.Model):
    ...
    club = models.ForeignKey(
        AquaristClub, on_delete=models.CASCADE, editable=False, related_name="member_clubs"
    )  # deletes species instances if user deleted
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, editable=False, related_name="user_club_members"
    )  # deletes species instances if user deleted
...
class CaresApprover(models.Model):
    ...
    approver = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="cares_approvers"
    )  # deletes species instances if user deleted
    specialty = models.CharField(
        max_length=5, choices=Species.CaresFamily.choices, default=Species.CaresFamily.UNDEFINED
    )
    last_updated = models.DateTimeField(auto_now=True)
...
class CaresRegistration(models.Model):
    ...
    cares_approver = models.ForeignKey(
        CaresApprover, on_delete=models.SET_NULL, null=True, blank=True, related_name="approver_cares_registrations"
    )
    affiliate_club = models.ForeignKey(
        AquaristClub, on_delete=models.SET_NULL, null=True, blank=True, related_name="club_cares_registrations"
    )
    species = models.ForeignKey(
        Species, on_delete=models.SET_NULL, blank=True, null=True, related_name="species_registrations"
    )
    collection_location = models.ForeignKey(
    ...
    status = models.CharField(
        max_length=4, choices=CaresRegistrationStatus.choices, default=CaresRegistrationStatus.OPEN
    )
    asn_imported = models.BooleanField(default=False)
    external_id = models.PositiveIntegerField(null=True, blank=True, unique=True)

    last_updated_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="user_cares_registration_last_updates"
    )
...
class BapLeaderboard(models.Model):
    ...
    lastUpdated = models.DateTimeField(
        auto_now=True
    )  # compare dates of aquarist BAP submissions and only update when needed
...
class BapGenus(models.Model):
    ...
    example_species = models.ForeignKey(
        Species, on_delete=models.SET_NULL, null=True, related_name="example_bap_species"
    )
    species_count = models.PositiveIntegerField(
        default=0
    )  # cached value to eliminate N+1 queries in GenusPoints list view
    species_override_count = models.PositiveIntegerField(
        default=0
    )  # cached value to eliminate N+1 queries in GenusPoints list view
    created = models.DateTimeField(auto_now_add=True)
    lastUpdated = models.DateTimeField(auto_now=True)  # updated every save
...
class BapSpecies(models.Model):
    name = models.CharField(max_length=240)
    species = models.ForeignKey(
        Species, on_delete=models.CASCADE, null=True, related_name="bap_species"
    )  # deletes ALL instances referencing any deleted species
...
class ImportArchive(models.Model):
    name = models.CharField(max_length=240)
    aquarist = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, editable=False, related_name="aquarist_imports"
    )  # preserve import archives for deleted users
...
class SpeciesImportStaging(models.Model):
    ...
    existing_species = models.ForeignKey(
        Species, null=True, blank=True, on_delete=models.SET_NULL, related_name="import_staging"
    )
    ...
    reviewed_by = models.ForeignKey(
        User, null=True, blank=True, on_delete=models.SET_NULL, related_name="reviewed_imports"
    )
    reviewed_at = models.DateTimeField(null=True, blank=True)

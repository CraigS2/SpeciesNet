...
def is_valid_binomial(species_name: str) -> bool:
    ...
    return all(c.isalpha() or c == "-" for c in genus + species)
...
def get_fishbase_url(species_name: str) -> str | None:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "distribution" in content or "biology" in content or "classification" in content:
                ...
                return response.url
            print("WARNING: Page exists but appears to be error/not found page")
            return None
        if response.status_code == 404:
            print("WARNING: Species not in FishBase (404)")
            return None
...
def extract_distribution(soup: BeautifulSoup) -> str | None:
...
def extract_biology(soup: BeautifulSoup) -> str | None:
...
def extract_conservation_notes(soup: BeautifulSoup) -> str | None:
...
def extract_iucn_from_fishbase(soup: BeautifulSoup) -> str | None:
...
def extract_common_name(soup: BeautifulSoup) -> str | None:
...
def get_fishbase_data(fishbase_url: str) -> dict[str, str]:
    ...
    result = {"common_name": "", "distribution": "", "biology": "", "conservation_notes": "", "iucn_status": ""}
...
def enrich_species_row(row: dict[str, str]) -> tuple[dict[str, str], str | None]:
    ...
    try:
        ...
        if fishbase_url:
            ...
            try:
                print("Extracting CLEAN data from FishBase page")
...
def collect_species_data_as_csv(import_archive: ImportArchive, current_user: User):
    ...
    with open(import_archive.import_csv_file.path, encoding="utf-8") as input_csv_file:
        ...
        tprint(f"\n{'=' * 60}")
        tprint("All Species Data Collection COMPLETED")

...
def get_matching_cares_approver(species):
    ...
    if specialty_matches.exists():
        ...
        logger.info(
            'resolve_cares_approver: matched CaresApprover "%s" (id=%s) by specialty "%s" for species "%s".',
            cares_approver.name,
            cares_approver.id,
            cares_family,
            species.name,
...
def get_notification_approvers(species):
    ...
    return CaresApprover.objects.filter(Q(specialty=species.cares_family) | Q(specialty="UDF")).select_related(
        "approver"
    )

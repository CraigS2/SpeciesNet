...
class SpeciesInstanceCreateViewTest(TestCase):
    ...
    def test_create_species_instance_with_valid_optional_data(self):
        ...
        self.client.login(username="testuser", password="testpass123")
        self.client.post(
            reverse("createSpeciesInstance", args=[self.species.id]),
            {
                "name": "Hoo Dunnit",
...
class SpeciesInstanceEditViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.species_instance = SpeciesInstance.objects.create(
            name="My Auratus", user=self.user, species=self.species, unique_traits="Bright colors", year_acquired=2023
        )
    ...
    def test_edit_species_instance_with_valid_optional_data(self):
        ...
        response = self.client.post(
            reverse("editSpeciesInstance", args=[self.species_instance.id]),
            {
        ...
        if response.status_code != 302:
            form_errors = (
                response.context["form"].errors if response.context and "form" in response.context else "No form"
            )
...
class SpeciesInstanceDeleteViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.species_instance = SpeciesInstance.objects.create(name="My Auratus", user=self.user, species=self.species)

...
class SpeciesInstanceLogEntryViewTests(TestCase):
    ...
    def setUp(self):
        ...
        # Create test users
        self.user1 = User.objects.create_user(email="user1@test.com", username="testuser1", password="testpass123")
        self.user2 = User.objects.create_user(email="user2@test.com", username="testuser2", password="testpass123")
        self.staff_user = User.objects.create_user(
            email="staff@test.com", username="staffuser", password="testpass123", is_staff=True
        )
        self.admin_user = User.objects.create_user(
            email="admin@test.com", username="adminuser", password="testpass123", is_admin=True
        )

        # Create test species
        self.species = Species.objects.create(
            name="Aulonocara jacobfreibergi", common_name="Butterfly Peacock", category="CIC", global_region="AFR"
        )
        ...
        self.log_entry = SpeciesInstanceLogEntry.objects.create(
            name="Test Log Entry", speciesInstance=self.species_instance, log_entry_notes="Initial test notes"
        )
    ...
    def test_create_log_entry_post_valid_data(self):
        ...
        data = {"name": "2025-01-15 Test Log Entry", "log_entry_notes": "Test notes for new log entry"}

        response = self.client.post(url, data)
        self.assertEqual(response.status_code, 302)

        # Verify log entry was created
        self.assertTrue(SpeciesInstanceLogEntry.objects.filter(name="2025-01-15 Test Log Entry").exists())
    ...
    def test_edit_log_entry_post_valid_data(self):
        ...
        data = {"name": "Updated Log Entry", "log_entry_notes": "Updated test notes"}
    ...
    def test_delete_log_entry_post_deletes_entry(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertFalse(SpeciesInstanceLogEntry.objects.filter(id=log_entry_id).exists())
...
class SpeciesMaintenanceLogViewTests(TestCase):
    ...
    def setUp(self):
        ...
        # Create test species
        self.species = Species.objects.create(
            name="Aulonocara stuartgranti", common_name="Flavescent Peacock", category="CIC", global_region="AFR"
        )

        # Create test species instances
        self.species_instance1 = SpeciesInstance.objects.create(
            name="Test Species Instance 1", user=self.user1, species=self.species
        )

        self.species_instance2 = SpeciesInstance.objects.create(
            name="Test Species Instance 2", user=self.user2, species=self.species
        )

        # Create test maintenance log
        self.maintenance_log = SpeciesMaintenanceLog.objects.create(
            name="Test Maintenance Log", species=self.species, description="Test description"
        )
    ...
    def test_create_maintenance_log_post_valid_data(self):
        ...
        data = {"name": "New Maintenance Log", "description": "New test description", "log_is_private": False}

        response = self.client.post(url, data)
        self.assertEqual(response.status_code, 302)

        # Verify maintenance log was created
        self.assertTrue(SpeciesMaintenanceLog.objects.filter(name="New Maintenance Log").exists())
    ...
    def test_edit_maintenance_log_post_valid_data(self):
        ...
        data = {"name": "Updated Maintenance Log", "description": "Updated description", "log_is_private": True}
    ...
    def test_delete_maintenance_log_post_deletes_log(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertFalse(SpeciesMaintenanceLog.objects.filter(id=log_id).exists())
...
class SpeciesMaintenanceLogEntryViewTests(TestCase):
    ...
    def setUp(self):
        ...
        # Create test species
        self.species = Species.objects.create(
            name="Pseudotropheus saulosi", common_name="Saulosi Cichlid", category="CIC", global_region="AFR"
        )

        # Create test species instance
        self.species_instance = SpeciesInstance.objects.create(
            name="Test Species Instance", user=self.user1, species=self.species
        )
    ...
    def test_create_log_entry_post_valid_data(self):
        ...
        data = {"name": "2025-01-15 Test Maintenance Entry", "log_entry_notes": "Test maintenance notes"}

        response = self.client.post(url, data)
        self.assertEqual(response.status_code, 302)

        # Verify log entry was created
        self.assertTrue(SpeciesMaintenanceLogEntry.objects.filter(name="2025-01-15 Test Maintenance Entry").exists())
    ...
    def test_edit_log_entry_post_valid_data(self):
        ...
        data = {"name": "Updated Maintenance Entry", "log_entry_notes": "Updated maintenance notes"}
    ...
    def test_delete_log_entry_post_deletes_entry(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertFalse(SpeciesMaintenanceLogEntry.objects.filter(id=entry_id).exists())
    ...
    def test_maintenance_log_entry_updates_species_instance_timestamp(self):
        ...
        # Create new entry
        url = reverse("createSpeciesMaintenanceLogEntry", args=[self.maintenance_log.id])
        data = {"name": "Timestamp Test Entry", "log_entry_notes": "Testing timestamp update"}
...
class AdminUserPermissionTests(TestCase):
    ...
    def setUp(self):
        ...
        self.regular_user = User.objects.create_user(
            email="regular@test.com", username="regularuser", password="testpass123"
        )
        self.admin_only_user = User.objects.create_user(
            email="admin@test.com", username="adminuser", password="testpass123", is_admin=True, is_staff=False
        )
        self.staff_only_user = User.objects.create_user(
            email="staff@test.com", username="staffuser", password="testpass123", is_admin=False, is_staff=True
        )
        self.admin_and_staff_user = User.objects.create_user(
            email="both@test.com", username="bothuser", password="testpass123", is_admin=True, is_staff=True
        )
        ...
        self.species_instance = SpeciesInstance.objects.create(
            name="Test Instance", user=self.regular_user, species=self.species
        )

        self.log_entry = SpeciesInstanceLogEntry.objects.create(
            name="Test Entry", speciesInstance=self.species_instance, log_entry_notes="Test notes"
        )

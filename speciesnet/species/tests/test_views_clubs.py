...
class AquaristClubCreateViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.regular_user = User.objects.create_user(
            email="user@test.com", username="regularuser", password="testpass123"
        )
...
class AquaristClubEditViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.non_member = User.objects.create_user(
            email="nonmember@test.com", username="nonmember", password="testpass123"
        )
        self.regular_member = User.objects.create_user(
            email="member@test.com", username="regularmember", password="testpass123"
        )
        ...
        # Create a club
        self.club = AquaristClub.objects.create(name="Test Club", acronym="TC", city="Test City", bap_default_points=10)
        ...
        self.admin_membership = AquaristClubMember.objects.create(
            name="TC: clubadmin", user=self.club_admin, club=self.club, is_club_admin=True, membership_approved=True
        )
        ...
        if response.status_code != 302:
            if hasattr(response, "context") and response.context and "form" in response.context:
                ...
                if hasattr(form, "errors"):
                    ...
                    self.fail("Form validation errors:\n" + "\n".join(error_details))
...
class AquaristClubMemberCreateViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.auto_club = AquaristClub.objects.create(
            name="Auto Approval Club", acronym="AAC", city="Test City", require_member_approval=False
        )

        # Create club requiring approval
        self.approval_club = AquaristClub.objects.create(
            name="Approval Required Club", acronym="ARC", city="Test City", require_member_approval=True
        )
...
class AquaristClubMemberEditViewTests(TestCase):
    ...
    def setUp(self):
        ...
        # Create club
        self.club = AquaristClub.objects.create(name="Test Club", acronym="TC", city="Test City")
    ...
    def test_edit_member_club_admin_can_approve_pending_member(self):
        ...
        pending_user = User.objects.create_user(
            email="pending@test.com", username="pendinguser", password="testpass123"
        )
        pending_membership = AquaristClubMember.objects.create(
            name="TC: pendinguser", user=pending_user, club=self.club, is_club_admin=False, membership_approved=False
        )
...
class AquaristClubMemberDeleteViewTests(TestCase):
    ...
    def test_delete_member_club_admin_can_remove_other_admins(self):
        ...
        another_admin = User.objects.create_user(
            email="anotheradmin@test.com", username="anotheradmin", password="testpass123"
        )
        another_admin_membership = AquaristClubMember.objects.create(
            name="TC: anotheradmin", user=another_admin, club=self.club, is_club_admin=True, membership_approved=True
        )
...
class ClubPermissionEdgeCaseTests(TestCase):
    ...
    def setUp(self):
        ...
        self.member_club_a = User.objects.create_user(
            email="membera@test.com", username="membera", password="testpass123"
        )
        self.admin_club_a = User.objects.create_user(email="admina@test.com", username="admina", password="testpass123")
        self.admin_club_b = User.objects.create_user(email="adminb@test.com", username="adminb", password="testpass123")

        # Create two clubs
        self.club_a = AquaristClub.objects.create(name="Club A", acronym="CA", city="City A")
        self.club_b = AquaristClub.objects.create(name="Club B", acronym="CB", city="City B")

        # Create memberships
        self.member_a = AquaristClubMember.objects.create(
            name="CA: membera", user=self.member_club_a, club=self.club_a, is_club_admin=False, membership_approved=True
        )
        self.admin_a = AquaristClubMember.objects.create(
            name="CA: admina", user=self.admin_club_a, club=self.club_a, is_club_admin=True, membership_approved=True
        )
        self.admin_b = AquaristClubMember.objects.create(
            name="CB:  adminb", user=self.admin_club_b, club=self.club_b, is_club_admin=True, membership_approved=True
        )

...
class ChooseSpeciesInstancesForLabelsViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.other_user = User.objects.create_user(email="other@test.com", username="other", password="testpass123")

        # Create species
        self.species1 = Species.objects.create(name="Aulonocara jacobfreibergi", category="CIC", global_region="AFR")
        self.species2 = Species.objects.create(name="Melanochromis auratus", category="CIC", global_region="AFR")

        # Create species instances for aquarist (currently keeping)
        self.instance1 = SpeciesInstance.objects.create(
            name="Aulonocara jacobfreibergi", user=self.aquarist, species=self.species1, currently_keep=True
        )
        self.instance2 = SpeciesInstance.objects.create(
            name="Melanochromis auratus", user=self.aquarist, species=self.species2, currently_keep=True
        )

        # Create species instance NOT currently keeping (should not appear)
        self.instance3 = SpeciesInstance.objects.create(
            name="Old Species", user=self.aquarist, species=self.species1, currently_keep=False
        )
    ...
    def test_choose_labels_post_saves_to_session(self):
        ...
        data = {"species": [str(self.instance1.id), str(self.instance2.id)]}
    ...
    def test_choose_labels_post_single_selection(self):
        ...
        data = {"species": [str(self.instance1.id)]}

        response = self.client.post(url, data)
        self.assertEqual(response.status_code, 302)

        # Check single selection in session
        self.assertEqual(self.client.session["species_choices"], [str(self.instance1.id)])
...
class EditSpeciesInstanceLabelsViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.instance = SpeciesInstance.objects.create(
            name="Aulonocara jacobfreibergi", user=self.aquarist, species=self.species, currently_keep=True
        )
    ...
    def test_edit_labels_reuses_existing_labels(self):
        ...
        self.assertEqual(SpeciesInstanceLabel.objects.filter(speciesInstance=self.instance).count(), 1)
    ...
    def test_edit_labels_multiple_species(self):
        ...
        species2 = Species.objects.create(name="Melanochromis auratus", category="CIC", global_region="AFR")
        instance2 = SpeciesInstance.objects.create(
            name="Melanochromis auratus", user=self.aquarist, species=species2, currently_keep=True
        )

        # Use workflow with both instances
        choose_url = reverse("chooseSpeciesInstancesForLabels", args=[self.aquarist.id])
        self.client.post(choose_url, {"species": [str(self.instance.id), str(instance2.id)]})
        ...
        self.assertTrue(SpeciesInstanceLabel.objects.filter(speciesInstance=self.instance).exists())
        self.assertTrue(SpeciesInstanceLabel.objects.filter(speciesInstance=instance2).exists())
...
class LabelWorkflowIntegrationTests(TestCase):
    ...
    def setUp(self):
        ...
        self.species1 = Species.objects.create(name="Test Species 1", category="CIC", global_region="AFR")

        self.species2 = Species.objects.create(name="Test Species 2", category="CIC", global_region="AFR")

        self.instance1 = SpeciesInstance.objects.create(
            name="Test Species 1", user=self.aquarist, species=self.species1, currently_keep=True
        )

        self.instance2 = SpeciesInstance.objects.create(
            name="Test Species 2", user=self.aquarist, species=self.species2, currently_keep=True
        )
    ...
    def test_complete_label_workflow_multiple_species(self):
        ...
        choose_url = reverse("chooseSpeciesInstancesForLabels", args=[self.aquarist.id])
        choose_data = {"species": [str(self.instance1.id), str(self.instance2.id)]}
